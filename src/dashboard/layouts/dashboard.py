...
def create_layout(current_user, seller_name: str = None):
    ...
    return html.Div(
        ...
        children=[
            # ── TOP BAR (LOGO | FILTERS | LOGOUT) ─────────────────────────────
            dbc.Row(className="g-2 mb-2 align-items-center filter-control-bar p-2 mx-0 rounded-3", children=[
                # Coluna 1: Logo & Título
                dbc.Col(xs=12, lg=2, children=[
                    html.H6("Sales Performance", className="mb-0 fw-bold"),
                    html.Small(f"Logado como: {username}", className="text-muted extra-small")
                ]),

                # Coluna 2: Filtros (dinâmicos por papel)
                dbc.Col(xs=12, lg=7, children=[
                    # SELLER: mostra apenas competência + badge com seu nome
                    dbc.Row(className="g-2", children=[
                        # Filtro Filial: visível apenas para ADMIN e MANAGER
                        dbc.Col(
                            dcc.Dropdown(
                        ...
                        # Filtro Vendedor: visível apenas para ADMIN e MANAGER
                        dbc.Col(
                            dcc.Dropdown(
                        ...
                        # Filtro Competência: sempre visível
            ...
            dbc.Modal([
                ...
                dbc.ModalFooter([
                    dbc.Button(
                        [html.I(className="fa-solid fa-xmark me-2"), "Cancelar"],
                        id="profile-modal-cancel", color="secondary", size="sm", outline=True
                    ),
                    dbc.Button(
                        [html.I(className="fa-solid fa-floppy-disk me-2"), "Salvar Senha"],
                        id="profile-save-password", color="primary", size="sm", className="fw-bold"
                    ),
                ])
